...
9/19(월) 평일 공부 시간 목표: 4 뽀모도리
9/19(월) 평일 실제 공부 시간: 4 뽀모도리
...
document.querySelector("#try").onkeypress = function(e){
    if(e.keycode == 13 || e.which == 3){ // 파이어폭스는 which로 확인함.
        finding();
        return false;
    }
}
...
function prevSlide(){
    if (showPicIndex == 0){ // 맨 앞에서 앞으로 버튼을 누르면, 맨 뒤로
...
function nextSlide(){
    if (showPicIndex == pics.length - 1){ // 맨 뒤에서 뒤로 버튼을 누르면, 맨 앞으로
        showPicIndex = 0;
    }
    else{
        showPicIndex++;
